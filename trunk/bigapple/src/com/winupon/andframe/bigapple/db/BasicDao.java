...
public class BasicDao {
	...
	protected long insert(String table, String nullColumnHack,
		...
		lock.lock();
		try {
			updateCount = getSQLiteDatabase().insert(table, nullColumnHack,
					values);
		} catch (Exception e) {
			LogUtils.e("", e);
		} finally {
	...
	protected int update(String table, ContentValues values,
		...
		lock.lock();
		try {
			updateCount = getSQLiteDatabase().update(table, values,
	...
	protected int delete(String table, String whereClause, String[] whereArgs) {
		...
		lock.lock();
		try {
			updateCount = getSQLiteDatabase().delete(table, whereClause,
					whereArgs);
		} catch (Exception e) {
			LogUtils.e("", e);
		} finally {
	...
	protected <T> List<T> query(String table, String[] columns,
		...
		lock.lock();
		Cursor cursor = getSQLiteDatabase().query(table, columns, selection,
	...
	protected <T> List<T> query(boolean distinct, String table,
		...
		lock.lock();
		Cursor cursor = getSQLiteDatabase().query(distinct, table, columns,
	 ...
	 * @return
	 */
	protected <T> List<T> query(String sql, String[] args,
			MultiRowMapper<T> multiRowMapper) {
		List<T> ret = new ArrayList<T>();
	 ...
	 * @return
	 */
	protected <T> T query(String sql, String[] args,
	 ...
	 * @return
	 */
	protected <K, V> Map<K, V> query(String sql, String[] args,
	 ...
	 * @return
	 */
	protected <T> List<T> queryForInSQL(String prefix, String[] prefixArgs,
