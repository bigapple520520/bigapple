...
public class BitmapCacheManager {
    ...
    public static BitmapCacheManager getInstance(BitmapCache bitmapCache) {
        if (null == bitmapCache) {
            throw new NullPointerException(
                    "when call BitmapCacheManager.getInstance the params bitmapCache can not be null!");
        }
        ...
        protected Integer doInBackground(Object... params) {
            ...
            try {
                ...
                case MESSAGE_CLEAR_BY_KEY:
                    bitmapCache.clearCache(String.valueOf(params[1]), (BitmapDisplayConfig) params[2]);
                    break;
                case MESSAGE_CLEAR_MEMORY_BY_KEY:
                    bitmapCache.clearMemoryCache(String.valueOf(params[1]), (BitmapDisplayConfig) params[2]);
                    break;
    ...
    public void clearCache(String uri, BitmapDisplayConfig config, AfterClearCacheListener listener) {
        new BitmapCacheManagementTask().execute(BitmapCacheManagementTask.MESSAGE_CLEAR_BY_KEY, uri, config, listener);
    }

    public void clearMemoryCache(String uri, BitmapDisplayConfig config, AfterClearCacheListener listener) {
