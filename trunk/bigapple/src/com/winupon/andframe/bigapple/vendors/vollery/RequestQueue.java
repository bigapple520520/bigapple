 ...
 * 
 * Calling {@link #add(Request)} will enqueue the given Request for dispatch, resolving from either cache or network on
 * a worker thread, and then delivering a parsed response on the main thread.
 */
public class RequestQueue {
     ...
     * <li>get(cacheKey) returns waiting requests for the given cache key. The in flight request is <em>not</em>
     * contained in that list. Is null if no requests are staged.</li>
     * </ul>
     */
    private final Map<String, Queue<Request<?>>> mWaitingRequests = new HashMap<String, Queue<Request<?>>>();

    /**
     * The set of all requests currently being processed by this RequestQueue. A Request will be in this set if it is
     * waiting in any queue or currently being processed by any dispatcher.
     */
    /** 所有的请求都会放在这里 */
    ...
    /** The network dispatchers. 工作线程 */
    private NetworkDispatcher[] mDispatchers;

    /** The cache dispatcher. 缓存线程 */
    ...
    public <T> Request<T> add(Request<T> request) {
        ...
        synchronized (mWaitingRequests) {
            ...
            if (mWaitingRequests.containsKey(cacheKey)) {
                ...
                // 相同cacheKey的请求在等待，那么就挂在这个cacheKey后面
