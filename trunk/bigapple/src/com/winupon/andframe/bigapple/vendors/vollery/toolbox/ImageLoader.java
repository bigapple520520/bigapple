 ...
 */
package com.winupon.andframe.bigapple.vendors.vollery.toolbox;
...
import com.winupon.andframe.bigapple.vendors.vollery.Request;
import com.winupon.andframe.bigapple.vendors.vollery.RequestQueue;
import com.winupon.andframe.bigapple.vendors.vollery.Response.ErrorListener;
import com.winupon.andframe.bigapple.vendors.vollery.Response.Listener;
import com.winupon.andframe.bigapple.vendors.vollery.VolleyError;
 ...
 * image listener provided by {@link ImageLoader#getImageListener(ImageView, int, int)}. Note that all function calls to
 * this class must be made from the main thead, and all responses will be delivered to the main thread as well.
 */
public class ImageLoader {
    ...
    /**
     * HashMap of Cache keys -> BatchedImageRequest used to track in-flight requests so that we can coalesce multiple
     * requests to the same URL into a single network request.
     */
     ...
     * Volley. Implementations must not block. Implementation with an LruCache is recommended.
     */
    ...
    /**
     * The default implementation of ImageListener which handles basic functionality of showing a default image until
     * the network response is received, at which point it will switch to either the actual image or the error image.
     * 
     * @param view
     ...
     * 
     * The call flow is this: 1. Upon being attached to a request, onResponse(response, true) will be invoked to reflect
     * any cached data that was already available. If the data was available, response.getBitmap() will be non-null.
     * 
     * 2. After a network response returns, only one of the following cases will happen: - onResponse(response, false)
     * will be called if the image was loaded. or - onErrorResponse will be called if there was an error loading the
     * image.
     */
    public interface ImageListener extends ErrorListener {
         ...
         * @param isImmediate
         *            True if this was called during ImageLoader.get() variants. This can be used to differentiate
         *            between a cached image loading and a network image loading in order to, for example, run an
         *            animation to fade in network loaded images.
         */
        public void onResponse(ImageContainer response, boolean isImmediate);
    }
     ...
     * 
     * The ImageContainer will contain either the specified default bitmap or the loaded bitmap. If the default was
     * returned, the {@link ImageLoader} will be invoked when the request is fulfilled.
     * 
     * @param requestUrl
    ...
    /**
     * Issues a bitmap request with the given URL if that image is not available in the cache, and returns a bitmap
     * container that contains all of the data relating to the request (as well as the default image if the requested
    ...
    /**
     * Sets the amount of time to wait after the first response arrives before delivering all responses. Batching can be
     * disabled entirely by passing in 0.
     * 
