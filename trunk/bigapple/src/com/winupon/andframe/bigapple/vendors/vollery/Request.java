...
import com.winupon.andframe.bigapple.vendors.vollery.VolleyLog.MarkerLog;
...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    /**
     * When a request can be retrieved from cache but must be refreshed from the network, the cache entry will be stored
     * here so that in the event of a "Not Modified" response, we can be sure it hasn't been evicted from cache.
     */
    ...
    /**
     * Creates a new request with the given URL and error listener. Note that the normal response listener is not
     * provided here as delivery of responses is provided by subclasses, who have a better idea of how to deliver an
    ...
    /**
     * Returns a list of extra HTTP headers to go along with this request. Can throw {@link AuthFailureError} as
    ...
    /**
     * Returns a Map of POST parameters to be used for this request, or null if a simple GET should be used. Can throw
     ...
     * 
     * <p>
     * By default, the body consists of the request parameters in application/x-www-form-urlencoded format. When
     * overriding this method, consider overriding {@link #getBodyContentType()} as well to match the new body format.
     * 
     * @throws AuthFailureError
     *             in the event of auth failure
     */
    ...
    /**
     * Returns the socket timeout in milliseconds per retry attempt. (This value can be changed per retry attempt if a
     * backoff is specified via backoffTimeout()). If there are no retry attempts remaining, this will cause delivery of
     * a {@link TimeoutError} error.
     */
    ...
    /**
     * Mark this request as having a response delivered on it. This can be used later in the request's lifetime for
     * suppressing identical responses.
     */
    ...
    /**
     * Subclasses must implement this to parse the raw network response and return an appropriate response type. This
     * method will be called from a worker thread. The response will not be delivered if you return null.
     * 
     * @param response
