 ...
 * 默认文件保存路径：/data/data/<packageName>/shared_prefs/<packageName>_preferences.xml<br>
 * 该类已单例模式存在，获取单例方式请调用BPPreferences.instance()方法
 * 
 * @author xuan
 * @version $Revision: 32003 $, $Date: 2012-10-31 14:00:09 +0800 (星期三, 31 十月
 *          2012) $
 */
public class BPPreferences {
	private SharedPreferences prefs;
	private Editor prefsEdit;
	private static BPPreferences instance;

	private BPPreferences() {
	}
	 ...
	 * @return
	 */
	public static BPPreferences instance() {
		if (null == instance) {
			instance = new BPPreferences();
			instance.prefs = PreferenceManager
					.getDefaultSharedPreferences(Bigapple
